...
public class ModuleDiscoveryService {
    ...
    private String buildDiscoveryPrompt(UseCaseGenerationContext context) {
        ...
        sb.append("<req_data>\n");
        sb.append("REQUIREMENTS TO ANALYZE:\n");
    ...
    private ModuleDiscoveryResult parseDiscoveryResult(JsonNode root, UseCaseGenerationContext context) {
        ...
        // Strict fallback logic: If unassigned requirements exist, we fail. (The retry logic could be implemented in orchestrator)
        if (!result.getUnassignedRequirementIds().isEmpty()) {
            throw new RuntimeException("AI failed to assign requirements: " + result.getUnassignedRequirementIds());
        }
