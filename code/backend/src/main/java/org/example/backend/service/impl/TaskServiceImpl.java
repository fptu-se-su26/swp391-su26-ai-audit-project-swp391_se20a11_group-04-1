...
public class TaskServiceImpl implements TaskService {
    ...
    private void applyRequest(Task task, TaskRequest request, Long projectId) {
        ...
        if (request.getRequirementId() != null) task.setRequirementId(request.getRequirementId());
    ...
    private String requiredText(String value, String message) {
        if (value == null || value.trim().isEmpty()) {
            throw new CustomException.BadRequestException(message);
        }
        return value.trim();
    }
    ...
    private <T extends Enum<T>> T parseEnum(String value, Class<T> enumType, T fallback) {
        ...
        try {
            ...
            throw new CustomException.BadRequestException("Invalid " + enumType.getSimpleName() + ": " + value);
        }
    }
