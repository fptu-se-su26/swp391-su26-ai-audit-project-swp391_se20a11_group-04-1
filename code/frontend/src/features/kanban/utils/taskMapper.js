...
export const mapTaskFromApi = (task) => ({
  ...
  sprint: task.sprintId ? `Sprint ${task.sprintId}` : 'No Sprint',
  sprintId: task.sprintId || null,
  assignee: buildAssignee(task.primaryAssignee),
  requirement: task.requirementId ? `REQ-${String(task.requirementId).padStart(2, '0')}` : 'REQ-NEW',
...
const parseSprintId = (value) => {
  if (!value || value === 'No Sprint') return null
  const match = String(value).match(/\d+/)
  return match ? Number(match[0]) : null
}
...
export const mapTaskToApi = (payload) => ({
  ...
  requirementId: parseRequirementId(payload.requirement),
  sprintId: parseSprintId(payload.sprint),
