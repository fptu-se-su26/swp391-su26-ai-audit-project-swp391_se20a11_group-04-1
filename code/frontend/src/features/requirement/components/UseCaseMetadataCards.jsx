  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter mb-stack_lg">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-stack_md flex items-center gap-4 hover:shadow-sm transition-shadow">
        <div className="w-12 h-12 rounded-full bg-[#1E707D]/10 flex items-center justify-center text-[#1E707D] shrink-0">
          <span className="material-symbols-outlined">description</span>
        </div>
        <div>
          <p className="font-label-md text-label-md text-secondary uppercase mb-1">Linked Requirement</p>
          <Link to={`/projects/${projectId}/requirements/${useCase.requirementId}`} className="font-body-lg text-body-lg text-[#1E707D] hover:underline font-semibold flex items-center gap-1">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-stack_md flex items-center gap-4 hover:shadow-sm transition-shadow">
        <div className="w-12 h-12 rounded-full bg-tertiary-container flex items-center justify-center text-on-tertiary-container shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {isEditing ? (
              <div className="w-full flex flex-wrap gap-1 p-1 border border-outline-variant rounded-lg bg-surface-container-lowest focus-within:border-[#1E707D] focus-within:ring-1 focus-within:ring-[#1E707D] transition-all">
                {(useCase.actors || []).map((actor, idx) => (
                  <span key={idx} className="flex items-center gap-1 bg-[#1E707D]/10 text-[#1E707D] px-2 py-0.5 rounded-full text-sm">
                    {actor}
                    <button 
                      onClick={() => handleRemoveActor(idx)}
                      className="text-[#1E707D] hover:text-error rounded-full"
                    >
                      &times;
                    </button>
                  </span>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {useCase.actors && useCase.actors.length > 0 ? (
                  useCase.actors.map((actor, idx) => (
                    <span key={idx} className="bg-[#1E707D]/10 text-[#1E707D] px-2 py-0.5 rounded-full text-sm">
                      {actor}
                    </span>
                  ))
                ) : (
      ...
      {/* AI Confidence */}
      <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-stack_md flex items-center gap-4 hover:shadow-sm transition-shadow">
        <div className="w-12 h-12 rounded-full bg-[#1E707D]-container flex items-center justify-center text-[#1E707D] shrink-0">
        ...
        <div className="w-full">
          ...
          <div className="flex items-center gap-2">
            <div className="w-full h-2 bg-surface-container-highest rounded-full max-w-[96px]">
              <div 
                className={`h-full rounded-full ${useCase.completenessScore < 50 ? 'bg-error' : 'bg-[#1E707D]'}`} 
