...
export default function ResourceTab({ classroomId, classroomData, userId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Có lỗi xảy ra khi thêm tài liệu');
    } finally {
  ...
  const handleDelete = async (resourceId) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Có lỗi xảy ra khi xóa tài liệu');
    }
  };
  ...
  const handleDownload = async (resource) => {
    ...
    try {
      ...
      toast.success('Tải xuống bắt đầu!', { id: toastId });
    } catch (err) {
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800">Thêm Tài Liệu</h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              {uploadType === 'FILE' ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Tên tài liệu</label>
                    <input 
                      type="text" 
                      value={fileName}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Chọn file (PDF, Word, Excel, Hình ảnh, Slide)</label>
                  <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:bg-slate-50 transition-colors relative">
                    ...
                    <p className="text-xs text-slate-500 mt-1">Hệ thống sẽ tự động nén file thành .zip khi người dùng tải xuống</p>
                  </div>
                </div>
              </div>
              ) : (
