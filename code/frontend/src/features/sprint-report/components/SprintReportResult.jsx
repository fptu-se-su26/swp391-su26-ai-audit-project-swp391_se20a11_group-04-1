...
export default function SprintReportResult({
  ...
  const liveSummary = selectedReport?.summary
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              { label: 'Overdue Tasks', value: displayMetrics.totalOverdueTasks ?? 0, icon: 'schedule', tone: 'text-amber-600', bg: 'bg-amber-50' },
              { label: 'Penalized Tasks', value: displayMetrics.totalPenalizedTasks ?? 0, icon: 'gavel', tone: 'text-rose-700', bg: 'bg-rose-50/50' },
            ].map((metric) => (
          ...
          {liveSummary && (
            <div className="rounded-xl bg-gradient-to-r from-slate-900 to-slate-800 p-6 shadow-md text-white">
              ...
              <p className="whitespace-pre-wrap text-base font-medium leading-relaxed text-slate-200">
                {liveSummary}
              </p>
            </div>
          )}
