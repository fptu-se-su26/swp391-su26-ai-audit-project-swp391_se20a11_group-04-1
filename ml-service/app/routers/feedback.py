...
from app.model_loader import append_feedback_entry, get_registry, strong_positive_count
...
def receive_recovery_signal(req: RecoverySignalRequest,
    ...
    append_feedback_entry(reg, entry)
    ...
    # Auto-trigger FAISS rebuild only after enough strong positive signals.
    if strong_count >= MIN_SIGNALS_TO_AUTO_RETRAIN:
        ...
        if not _training_status["running"]:
            ...
            log.info(
                "Auto-triggered FAISS retraining (buffer=%d strong_positive=%d)",
